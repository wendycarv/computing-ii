...
Status queue_insert(QUEUE hQ, int size, int num_in_line);
...
void test_case(Node** pHead)
{
    ...
    for (i = 0; i < num_of_cars; i++)
    {
        ...
        {
            queue_insert(hLeft, car_length, i + 1); //the queue and its number in line
        }
        
        if (strcmp(side, "right") == 0)
        {
            queue_insert(hRight, car_length, i + 1);
        }
    }
...
Status queue_insert(QUEUE hQ, int size, int num_in_line) //side, car length and # in line ... inserting at tail
{
    Queue* pQ = (Queue*)hQ;
